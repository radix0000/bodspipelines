...
from bodspipelines.transforms.gleif import Gleif2Bods, generate_statement_id
...
def test_lei_transform(lei_data):
    ...
    for bods_data in transform.process(lei_data, 'lei2'):
        ...
        assert validate_datetime(bods_data['publicationDetails']['publicationDate'])
...
def test_rr_transform(rr_data):
    ...
    for bods_data in transform.process(rr_data, 'rr'):
        ...
        assert bods_data['statementDate'] == '2018-03-23T14:07:34+00:00'
        assert bods_data['subject'] == {'describedByEntityStatement': '7d72f25e-910f-1f28-714d-7a761cbbc5de'}
        assert bods_data['interestedParty'] == {'describedByEntityStatement': 'c892c662-2904-0c7d-5978-07108d102c33'}
        assert bods_data['interests'] == [{'type': 'otherInfluenceOrControl',
...
def test_repex_transform_non_consolidating(repex_data_non_consolidating):
    ...
    for bods_data in transform.process(repex_data_non_consolidating, 'repex'):
        ...
        if bods_data['statementType'] == 'entityStatement':
            ...
            assert bods_data['interestedParty'] == {'unspecified': {'describedByEntityStatement': statementID}}
            assert bods_data['interests'] == [{'type': 'unknownInterest',
...
def test_repex_transform_natural_persons(repex_data_natural_persons):
    ...
    for bods_data in transform.process(repex_data_natural_persons, 'repex'):
        ...
        if bods_data['statementType'] == 'personStatement':
            ...
            assert bods_data['interestedParty'] == {'unspecified': {'describedByPersonStatement': statementID}}
            assert bods_data['interests'] == [{'type': 'unknownInterest',
                                               'interestLevel': 'direct',
                                               'beneficialOwnershipOrControl': False,
                                               'details': 'A controlling interest.'}]
